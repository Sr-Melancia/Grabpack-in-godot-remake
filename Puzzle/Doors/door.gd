...
@onready var basic_interaction = $"frame/door/Basic interact"
...
signal opened
signal closed
signal locked_attemp

func _unlock():
	basic_interaction._got_key()

func _ready():
	if locked:
		$"frame/door/Basic interact"._lock()
...
func _toggle_door():
	if locked:
		_lock_sfx()
		anim.play("locked")
		emit_signal("locked_attemp")
	else:
		if not open:
			basic_interaction._interactable()
			opening.play()
			anim.play("open")
			open = true
		else:
