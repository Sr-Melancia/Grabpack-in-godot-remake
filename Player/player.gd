...
func _ready():
	can_shoot_boot = true
	if has_flashlight:
		new_pack_anim.play("Armature|A_FirstPersonPlayer_FlashlightOn", -1, 20)
	if not has_flashlight:
		new_pack_anim.play("Armature|A_FirstPersonPlayer_FlashlightOff", -1, 20)
...
@onready var hand_changer = $"ui/Hand changer"
@onready var blocker_anim = $neck/head/grabpack_1/scale/pack_nodes/NewGrabpack/Sketchfab_model/f0286edbe6d648398062232b73231a0b_fbx/Object_2/RootNode/Armature/Object_6/Skeleton3D/Object_9/CPUParticles3D
...
func _power_green_line():
	green_line.visible = true
	line_r.visible = false

func _unpower_green_line():
	green_line.visible = false
	line_r.visible = true

func _block_hand():
	blocker_anim.emitting = true
	can_shot = false
	...
	if can_pack:
		if grabpack_version == 1 and r_hand_locked:
			...
			if Input.is_action_just_pressed("hand_switch_down"):
				_switch_hand_down()
			if Input.is_action_just_pressed("0") and red_hand and not Player.green_powered and can_shot:
				hand_changer._red()
				_switch_hand(1)
			if Input.is_action_just_pressed("1") and green_hand and not Player.green_powered and can_shot:
				hand_changer._green()
				_switch_hand(2)
			if Input.is_action_just_pressed("2") and rocket_hand and not Player.green_powered and can_shot:
				hand_changer._purple()
				_switch_hand(3)
			if Input.is_action_just_pressed("3") and flare_hand and not Player.green_powered and can_shot:
				hand_changer._flare()
				_switch_hand(4)
			if Input.is_action_just_pressed("4") and dash_hand and not Player.green_powered and can_shot:
				hand_changer._dash()
				_switch_hand(5)
...
var can_shoot_boot = true

func _process(delta):
	if can_shoot_boot:
		can_shoot_right = true
		can_shoot_left = true
	if not can_shoot_boot:
...
func _switch_hand(hand):
	if not hand_switch_anim:
		...
		new_pack_anim.play("Armature|A_FirstPersonPlayer_HandSwitch", -1, 2.5)
		switched_hand = hand
		switch_timer.start()
		hand_switch_anim = true
